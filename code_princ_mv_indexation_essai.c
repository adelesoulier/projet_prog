...
unsigned long exactocasei(double longi, double lat){
    ...
    lat = 359 - (lat + 89.75)*2; //conversion de la latitude en la ligne (donne un entier pour tous les .25 et .75)
    // on veut avoir le i = 0 tout en haut à gauche
...
void fctLatInit(double * latInit){
    // on va du Nord au Sud (de haut en bas) donc:
    int I1 = lat2i1D(40); // 40°N est la coordonnée à partir de laquelle notre gradient de déchets va commencer
    // (donc depuis le haut, le Nord)
    int I2 = lat2i1D(36); // à 36°N on est à saturation
    int I3 = lat2i1D(28); // à partir de là on va commencer à diminuer en concentration
    int I4 = lat2i1D(25); // on finit notre gradient de pollution à 25°
    ...
    for (int i = 0; i < 360; i++){
        ...
        // si on est au-delà des limites extérieures, la concentration initiale en plastiques est nulle
            ...
            // si on est entre I1 et I2, on va du Nord au Sud depuis 40°N, le gradient de plastiques augmente
            ...
            // si on se situe au Sud de la partie saturée, le gradient de pollution diminue
...
int main(int argc, char * argv[]) {
    ...
    fctLongInit(longInit); //(vecteur 1D pour les longitudes avec les concentrations initiales, un maximum à 1)
    fctLatInit(latInit); // idem pour  les latitudes
